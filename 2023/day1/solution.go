...
var validStringNumbers = map[string]int{
	"one":   1,
	"two":   2,
	"three": 3,
	"four":  4,
...
var validStringNumbersKeys = make([]string, 0, len(validStringNumbers))
...
fivetczxxvjrrqfive1sevennvj6one3`
	...
	for k := range validStringNumbers {
		validStringNumbersKeys = append(validStringNumbersKeys, k)
	}
	...
	for _, v := range splits {
		numbersFound := []string{}
		var currentValidNumbers []string = nil
		var result int = 0
		currentKeyRegistry := ""

		for j := 0; j < len(v); j++ {
			char := v[j]
			charIsNumber := unicode.IsNumber(rune(char))
			if charIsNumber {
				currentKeyRegistry = ""
				numbersFound = append(numbersFound, string(char))
			} else {
				if currentValidNumbers == nil {
					currentValidNumbers = validStringNumbersKeys
				}
				if len(currentKeyRegistry) == 0 {
					currentKeyRegistry = string(char)
				} else {
					currentKeyRegistry = fmt.Sprintf("%s%s", currentKeyRegistry, string(char))
				}

				currentValidNumbers, result = filterValidNumbers(currentValidNumbers, currentKeyRegistry)
				if result != 0 {
					numbersFound = append(numbersFound, fmt.Sprint(result))
					currentKeyRegistry = string(char)
				} else {
					if len(currentValidNumbers) == 0 {
						if j != 0 {
							currentKeyRegistry = fmt.Sprintf("%s%s", string(v[j-1]), string(char))
						} else {
							currentKeyRegistry = string(char)
						}
					}
		...
		finalResultStr := fmt.Sprintf("%s%s", numbersFound[0], numbersFound[len(numbersFound)-1])
		finalResultInt, _ := strconv.Atoi(finalResultStr)
		sum += finalResultInt
	}
	fmt.Println("the sum is ", sum)
}

func filterValidNumbers(filterFrom []string, prefix string) (possibleResults []string, value int) {
	var result []string
	for _, num := range filterFrom {
		if strings.HasPrefix(num, prefix) {
			result = append(result, num)
		}
	}
	// if the only result is one and result is
	if len(result) == 1 && result[0] == prefix {
		possibleResults = nil
		value = validStringNumbers[prefix]
	} else {
